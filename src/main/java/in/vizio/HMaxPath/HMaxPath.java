...
//	}
	...
	// 1. 	determine the 4 connected saddle: iterate on the image with diamond tip shape
	// 		and keep a list of saddle up points (maxima to be)
...
//			}	
	...
	public Collection<Point> getMinPathStart3( Point saddle , final RandomAccessibleInterval< T > source ) {
		...
		if( lab7>0 && lab4>0 ) {
			...
			//for(int i=0 ; i<8 ; i++ )
			//	labels[i] = labels[i]==labMax ? labMin : labels[i] ; 
			//newLabel--;
		}
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// test if one of the min path start is a 4C minimum and its 8C continuation merge with another min region 
		...
		for( Entry<Integer,Point> entry : minPathStarts.entrySet() ) {
			...
			if( step1.equals( step0 ) ) { // if step is not a 4C min
				neigh8CRA.setPosition( step0  );
				final Cursor<T> neighborhood2 = neigh8CRA.get().cursor();
				while( neighborhood2.hasNext() ) {
					float val = neighborhood2.next().getRealFloat();
...
//	}
	...
	public static void main(final String... args) throws IOException
	{
		...
		Dataset dataset = (Dataset) ij.io().open("C:/Users/Ben/workspace/testImages/saddle_test/blobs_close_up.tif");
